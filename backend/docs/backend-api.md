...
- POST /character-versions/
  - Client should send:
    - Required: character (UUID), prompt_snapshot (JSON object), aspect_ratio (string), generation_job_id (UUID)
    - Optional: generated_image (file/image; nullable for failed generations), thumbnail (file/image), seed (integer)
  - Accepted but backend-managed (avoid sending):
...
- GET|PUT|PATCH|DELETE /character-versions/<uuid:item_id>/
  - PUT client should send required: character (UUID), prompt_snapshot (JSON object), aspect_ratio (string), generation_job_id (UUID)
  - PUT optional: generated_image (file/image; nullable for failed generations)
  - PATCH client should send: only changed editable fields
...
- POST /cover-versions/
  - Client should send:
    - Required: book (UUID), title_text (string), prompt_snapshot (JSON object), aspect_ratio (string), generation_job_id (UUID)
    ...
    - Optional: generated_image (file/image; nullable for failed generations)
  - Accepted but backend-managed (avoid sending):
...
- GET|PUT|PATCH|DELETE /cover-versions/<uuid:item_id>/
  - PUT client should send required: book (UUID), title_text (string), prompt_snapshot (JSON object), aspect_ratio (string), generation_job_id (UUID)
  - PUT optional: generated_image (file/image; nullable for failed generations)
  - PATCH client should send: only changed editable fields
